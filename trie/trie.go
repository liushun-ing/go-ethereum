...
func (t *Trie) get(origNode node, key []byte, pos int) (value []byte, newnode node, didResolve bool, err error) {
	...
	case *shortNode:
		// 说明可能是扩展节点或者叶子结点，读取他的 value 然后进一步递归（value 可能指向一个分支节点，也可能是 value 节点，
		// 注意虽然他可能直接就是叶子结点，但是为了统一处理，叶子结点也是由 valueNode 构成，所以可以执行下一次递归得到他的值）。
