		...
		Subcommands: []*cli.Command{
			{
				...
				ArgsUsage: "<keyFile>",
				Action:    importWallet, // 在运行命令时要执行的函数
				Flags: []cli.Flag{ // 与命令一起使用的任何附加命令行标志
					utils.DataDirFlag,
...
func accountCreate(ctx *cli.Context) error {
	...
	// 底层是用 ecdsa.GenerateKey(crypto.S256(), rand) 生成私钥，然后得到公钥，最后得到地址
	// 然后构建好 key 对象之后，在加密一次，然后保存到文件中
...
// 更新账户，实际就是更换一下钱包密码，然后重新生成一次加密的 store 文件
