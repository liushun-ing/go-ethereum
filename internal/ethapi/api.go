...
// 立即执行新的消息调用，但不在区块链上创建交易。
// 通常用来执行只读智能合约的函数，例如 ERC-20 合约的 balanceOf 函数。
...
// 交易的 RPC 表示形式,transaction有很多实现，字段不一样，这里需要将他们全部统一到一起
...
// 在以太坊中，AccessList 是一种优化机制，用于改进交易的性能和效率。
// 具体来说，AccessList 指定了一个交易将要访问的存储位置和账户。
// 这可以减少交易执行期间的存储访问成本，提升整个网络的效率。
...
// 指的是到当前这个区块该用户一共发了多少交易，就是这个区块中账号的 nonce
func (api *TransactionAPI) GetTransactionCount(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (*hexutil.Uint64, error) {
	...
	nonce := state.GetNonce(address) // 就是返回他的 nonce 值
...
func (api *TransactionAPI) GetTransactionReceipt(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	...
	// Derive the sender.
	signer := types.MakeSigner(api.b.ChainConfig(), header.Number, header.Time) // signer 的作用是解析 from
...
// 提交交易到交易池，需要验证 gas 费是否合理，并返回交易 hash
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	// If the transaction fee cap is already specified, ensure the
	// fee of the given transaction is _reasonable_.
	// 检查 fee 是否合理 总的油费是否小于 1 ether
	...
	from, err := types.Sender(signer, tx) // 获取交易的发起者
	if err != nil {
		return common.Hash{}, err
	}
	...
	if tx.To() == nil {
		// 如果是发起合约交易，就创建一个合约地址
		// 可见，合约的地址就是交易发起者的地址和交易的 nonce 的 rlp 编码的 Keccak256 加密结果
...
// 对一个 bytes 数据签名，并返回签名后的 bytes
// 这个是原生的签名方法，实际还是用钱包的签名方法在签名
...
func (api *TransactionAPI) SignTransaction(ctx context.Context, args TransactionArgs) (*SignTransactionResult, error) {
	...
	// 为交易填充默认值，并自动估计油费限制 gasLimit 也就是 gas
	if err := args.setDefaults(ctx, api.b, false); err != nil {
		return nil, err
	}
	...
	signed, err := api.sign(args.from(), tx) // 签名
	if err != nil {
		return nil, err
	}
	...
	data, err := signed.MarshalBinary() // 得到二进制数据
	if err != nil {
		return nil, err
	}
	return &SignTransactionResult{data, signed}, nil
...
// 根据 hash 那交易，先从链上拿，如果没有，再从交易池中拿
...
// 检查交易费，要求：gasPrice * gas < cap (cap = 1 ether)
