...
// 这里的fees是tx.GasFeeCap - baseFee，也就是有效的矿工交易费
...
// 根据交易费和交易时间（也就是nonce）排序的交易列表，通过大顶堆实现的
...
func (t *transactionsByPriceAndNonce) Shift() {
	...
	if txs, ok := t.txs[acc]; ok && len(txs) > 0 {
		if wrapped, err := newTxWithMinerFee(txs[0], acc, t.baseFee); err == nil {
			t.heads[0], t.txs[acc] = wrapped, txs[1:] // 把第一个去掉了，然后用该账户的下一个交易来顶上
			heap.Fix(&t.heads, 0)
			return
		}
	}
