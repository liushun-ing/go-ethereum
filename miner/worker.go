...
type environment struct {
	...
	coinbase common.Address // 区块的邮费收获地址
...
func (miner *Miner) generateWork(params *generateParams) *newPayloadResult {
	...
	// 交给共识层去生成区块，这里应该要去尝试各种nonce，如果是POW的话，如果是POA的话，则直接生成区块
	// 随着以太坊从PoW转向PoS，普通的Geth节点不再直接参与挖矿活动，而是承担网络维护和交易处理的角色。
	// 挖矿活动已经被验证者系统取代，这些验证者通过质押ETH来参与区块生成和验证过程。
...
// commitTransactions 提交所有交易，包括普通交易和blob交易
...
// 从 txpool 中拿出所有的pending交易，分为 legacyPool 和 blobPool 交易
// 并且每个 pool 中又分为 local 交易个 remote 需要分开提交
// 然后提交执行交易
