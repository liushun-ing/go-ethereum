...
var DefaultConfig = Config{
	...
	// run 3 rounds.
	// 共识层通常等待区块生成的半个时段来生成 payload，这个时间内需要不断提交最新的 payload 信息，以保证数据可靠和一致
	// 并且需要保证三次提交，所以设置为2秒，包括pending中，也设置为了一个带提交的区块的有效期只有2秒。
...
// not initialized.
// 这个只是根据本地信息构建的一个pending状态的区块，并没有通过共识机制确认
...
// gasTip 就是 gasPrice，指的是矿工包含一个交易的最低gas价格
