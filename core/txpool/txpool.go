...
type TxPool struct {
	// 目前共有两种交易池，所以切片大小是 2：[]txpool.SubPool{legacyPool, blobPool}
	subpools []SubPool // List of subpools for specialized transaction handling

	reservations map[common.Address]SubPool // Map with the account to pool reservations 账户与池子的映射
...
// 创建一个交易池，包含两种类型的交易池，收集，排序，过滤（本地和网络）交易
...
// 生成一个映射器，返回一个函数，该函数由 subpool 调用，反过来更改 txpool 信息
// 函数的作用是判断 reservations 中是否包含有一个地址的池子映射信息，
// 并且有一个 reserve 标识来决定是否在reservations中继续保持该映射，否则就删除（如果有的话）
...
func (p *TxPool) Add(txs []*types.Transaction, local bool, sync bool) []error {
	...
	txsets := make([][]*types.Transaction, len(p.subpools)) // 将交易区分开来，分别记录两个池子的交易
	splits := make([]int, len(txs))                         // 保存每个交易都放到了哪个池子中
	...
	for i, tx := range txs {
		...
		for j, subpool := range p.subpools {
			// 就是根据交易的类型 tx.Type() 进行判断，共有四种交易类型
			// blobpool: types.BlobTxType
			// LegacyPool: types.LegacyTxType, types.AccessListTxType, types.DynamicFeeTxType
	...
	for i, split := range splits {
		...
		errsets[split] = errsets[split][1:] // 等价于移位运算
	}
	return errs
}
...
// 从 pending 找到所有现在可以执行的交易，按照 nonce 排序和 account 分组好
// 会根据传入的 filter 过滤一些不符合条件的，比如 gas 不够
...
// 返回所所有可执行的交易中的 nonce 最小的那一个，因为交易是需要按顺序打包的
