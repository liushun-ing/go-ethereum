...
var DefaultConfig = Config{
	...
	// 这也说明以太坊默认是不支持 0 GWei 交易的，还不太清楚？？？？
	// maybe：除非有个节点自己改了源码自己支持，但是别人要同步的时候，可能还是不支持。
	PriceLimit: 1,
	PriceBump:  10,

	// 注意，这些配置对本地交易无效
	// 可执行的默认配置：每个账户最多 16 个交易，一共最多 4096+1024 个交易，在 pending 中
	AccountSlots: 16,
	GlobalSlots:  4096 + 1024, // urgent + floating queue capacity with 4:1 ratio
	// 不可执行的默认配置：每个账户最多 64 个交易，一共最多 1024 个交易
	AccountQueue: 64,
	GlobalQueue:  1024,
...
type LegacyPool struct {
	...
	txFeed      event.Feed // 事件流，用于订阅事件后，向订阅者发消息
	signer      types.Signer
	mu          sync.RWMutex
	...
	journal *journal    // Journal of local transaction to back up to disk 本地交易会写入磁盘

	reserve txpool.AddressReserver // Address reserver to ensure exclusivity across subpools
	// pending 交易 pending存放可执行的交易
	pending map[common.Address]*list // All currently processable transactions
	// queue 交易 queued存放未来的、当前无法执行的交易
	queue  map[common.Address]*list     // Queued but non-processable transactions
	beats  map[common.Address]time.Time // Last heartbeat from each known account 每一个已知账号的最后一次心跳信息的时间
	all    *lookup                      // All transactions to allow lookups
	priced *pricedList                  // All transactions sorted by price 按照价格排序的交易
...
// 过滤 legacypool 支持的交易：types.LegacyTxType, types.AccessListTxType, types.DynamicFeeTxType
...
func (pool *LegacyPool) Init(gasTip uint64, head *types.Header, reserve txpool.AddressReserver) error {
	...
	// fully synced).
	statedb, err := pool.chain.StateAt(head.Root) // 获取当前 head 的块的世界状态的 db
	if err != nil {
	...
	for {
		...
		case <-evict.C:
			...
			for addr := range pool.queue {
				...
				// 如果最后一次心跳时间到现在的时间距离对于配置的不可执行的交易的最长持续时间，就删除
...
func (pool *LegacyPool) Close() error {
	...
	close(pool.reorgShutdownCh) // 直接关闭该通道，loop 就会监听到，就会直接 return 结束 loop
	pool.wg.Wait()
...
// LegacyPool.gasTip 就是 LegacyPool.Config.PriceLimit
// 更新最低的交易价格下限，并删除池中不符合条件的交易
...
// 得到所有 pending 中的所有交易：会根据 filter 进行过滤，如果 miner 要求了最低 fee 的话，但是 local 不受限制
...
func (pool *LegacyPool) validateTx(tx *types.Transaction, local bool) error {
	opts := &txpool.ValidationOptionsWithState{
		...
		// 这两个函数用于判断 balance 是否充足
		// 当前账户的 pending 交易的总花费
...
// 添加交易，如果存在了，不管，该交易不符合条件也不管，如果符合条件，就删除一些低价格交易，然后进行替换
...
// 注意这里是看是不是 gapped，返回的结果和是否是可执行的表示是相反的
...
// 将 local 交易写入磁盘，方便日后进行恢复，重启之后会重新导入
...
func (pool *LegacyPool) removeTx(hash common.Hash, outofbound bool, unreserve bool) int {
	...
	// 如果交易在 pending 中，把交易从 pending 删除， 并把因为这个交易的删除而变得无效的交易放到 queue
	if pending := pool.pending[addr]; pending != nil {
...
// 朝一个 reset 管道中发送消息，交给 loop 捕获执行，会在 scheduleReorgLoop 中触发 runReorg 然后会在那里面发布新交易事件，并广播出去
...
// 会在这里监听 LegacyPool 的 reqResetCh reqPromoteCh queueTxEventCh 事件，然后调用相关函数。
func (pool *LegacyPool) scheduleReorgLoop() {
	...
	for {
		...
		case tx := <-pool.queueTxEventCh:
			...
			// 可见，并不是每次有交易加入，都发布 sendTxEvt 事件
...
// 重新组织交易池， reset 会调整两个队列的交易，交易将会全部丢进 queue 中，并重置世界状态
// promoteExecutables 会将 queue 中的可执行的交易放入 pending 中，然后也会删除一些不合格的交易
...
// reset 一般是需要进行区块链同步的时候，才会触发：比如需要发布一个新块，或者需要同步别人的块，导致 block header 发生改变
// reset 会重新加载 new head 的世界状态，也会调整交易池，会将仍然可用的交易，重新放入 queue 池子，然后交给 promote 去判断哪些可执行
func (pool *LegacyPool) reset(oldHead, newHead *types.Header) {
	...
	if oldHead != nil && oldHead.Hash() != newHead.ParentHash {
		...
		if depth := uint64(math.Abs(float64(oldNum) - float64(newNum))); depth > 64 {
			...
			if rem == nil {
				...
				var discarded, included types.Transactions // rem/add 的交易
				// 新区块高度变小，需要删除老交易
				...
				// 所有在 discarded 中，但不在 included 的交易
				// 需要把这些交易重新放入池子，因为这些交易后面还需要用到，需要重新打包到区块
	...
	statedb, err := pool.chain.StateAt(newHead.Root) // 重新加载世界状态
	if err != nil {
	...
	pool.addTxsLocked(reinject, false) // 将交易加入 queue 中
}

// promoteExecutables moves transactions that have become processable from the
// future queue to the set of pending transactions. During this process, all
// invalidated transactions (low nonce, low balance) are deleted.
// 在重新 reset 之后，promoteExecutables 将会判断哪些交易是当前可执行的，会将其从 queue 提升到 pending 中
// 在这个过程中，会将一些没用的交易剔除
...
// 池子中所有的数据，分为 locals 和 remotes
type lookup struct {
	slots   int // 当前所有交易使用的 slot 总量
	lock    sync.RWMutex
	locals  map[common.Hash]*types.Transaction // 本地交易
	remotes map[common.Hash]*types.Transaction // 网络交易
}
...
// 将原本属于 local 的交易从 remotes 移入 locals（通过 locals 包含的本地账号信息判断）
