...
// 一个账户是以 stateObject 形式存在，为了便于账户隔离管理，使用不开放的 stateObject 来维护
...
// 将 dirtyStorage 提交到 pendingStorage 中，然后清空 dirtyStorage
...
func (s *stateObject) AddBalance(amount *uint256.Int, reason tracing.BalanceChangeReason) {
	...
	if amount.IsZero() {
		if s.empty() {
			s.touch() // 如果是这个空账户，则进行 touch。一旦账户被 touched ，则会在 Commit 时删除
		}
		return
	}
