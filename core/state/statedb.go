...
// commit states.
// 以太坊协议中的StateDB结构用于存储merkle trie中的任何内容。StateDB 负责缓存和存储嵌套状态。
// 一旦状态提交，在 stateDB 中缓存的尝试（包括帐户尝试、存储尝试）将不再起作用。
// 必须使用新的根和更新的数据库创建新的状态实例，以访问提交后状态。
type StateDB struct {
	// 操作状态的底层数据库，在实例化 StateDB 时指定
	db         Database
	prefetcher *triePrefetcher
	trie       Trie // 使用 originalRoot 拿到的 trie
	...
	originalRoot common.Hash // 构建 StateDB 时传入的 rootHash，这个就是一个区块的 hash
	...
	journal *journal // 日志就是一个数组
	// 记录了各个版本的日志流水的变更日志位置索引（其实就是当前版本号[nextRevisionId]和当前版本的日志流水长度[journal.length]的一个map）
...
// 要告诉 StateDB 需要使用哪个状态，因此需要提供 StateRoot 作为默克尔树根去构建树。
// StateRoot 值相当于数据版本号，根据版本号可以明确的知道要使用哪个版本的状态。
// 如果数据内容并没在树中，需要到一个数据库中读取。因此还需要提供 db 才能完成构建。
...
// 他一般是会在执行完一个交易后调用，将当前交易影响的账户从 dirty 提交到 pending，但是并不会持久化
...
// 只是收集状态修改和树更新，并重新设置所有的flags，各种缓存和新的状态
...
// 注意在commit中并没有写入到数据库，是在这里才调用的 rawdb.WriteCode
// 在commit中只是对缓存和内存中的数据做了处理，然后返回了需要更新的信息，如 更新的账户，删除的账户，和更新的 trieNode
