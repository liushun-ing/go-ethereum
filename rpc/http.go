...
// 创建一个 http 传输协议客户端
// 返回的是一个函数，可以通过回调函数拿到创建的 httpConn
...
// 发送一个 http 请求，然后解析出 response，并写入管道 op.resp
...
// 这个实际上和非 batch 没有很大差别， 都是发送一个 http 请求，只不过 body 是一个切片
...
// 使用 http 库发送一个 http 请求，并返回 resp.Body 数据，因为 rpc 只关注 body 数据
func (hc *httpConn) doRequest(ctx context.Context, msg interface{}) (io.ReadCloser, error) {
	body, err := json.Marshal(msg) // 将 json-rpc 数据序列化为字节
	if err != nil {
		return nil, err
	}
	// 构建 http 请求对象
