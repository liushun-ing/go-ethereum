...
// state scheme.
// triedb 操作数据的支持层，hashdb 或者 pathdb
...
// TrieDB => *triedb.Database 通过 backend 来执行操作，
// backend 有两种实现：hashdb 和 pathdb，这两个中会有 diskdb 对象，来操作底层数据库
type Database struct {
	config *Config // Configuration for trie database
	// 前映像，preimage 是一个哈希函数的输入数据，缓存 preimage 可以减少重复计算哈希值的开销，提高系统性能
